...
const mockLawFirms = [
  {
    ...
    description: "Profesjonalna kancelaria prawna specjalizująca się w prawie gospodarczym i cywilnym.",
    address: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (params.city) {
      ...
      filteredFirms = filteredFirms.filter((firm) => firm.address.city.toLowerCase().includes(city))
    }

    // Specialization filter
    if (params.specializations && params.specializations.length > 0) {
      filteredFirms = filteredFirms.filter((firm) =>
        firm.specializations.some((spec) => params.specializations!.includes(spec.code)),
      )
    }
    ...
    const paginatedFirms = filteredFirms.slice(offset, offset + params.per_page)
