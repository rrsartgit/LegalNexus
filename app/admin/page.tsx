...
type AdminDoc = { id: string; title: string; type: string; status: string; created_at: string }
      ...
      <main className="container mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
                <CardTitle>Pisma w systemie</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                {loading && <div>Ładowanie…</div>}
                {!loading && docs.length === 0 && <div>Brak pism</div>}
                {docs.map((d) => (
                  <div
                    key={d.id}
                    className="grid md:grid-cols-[1fr,120px,200px] gap-3 items-center border rounded p-3 bg-white"
                  >
                    <div>
                    ...
                    <div className="text-sm">Typ: {d.type}</div>
                    <div className="flex items-center gap-2">
                      <Select onValueChange={(v) => void updateStatus(d.id, v)} defaultValue={d.status}>
                      ...
                      <Button variant="outline" onClick={() => void updateStatus(d.id, "in_review")}>
                        Oznacz "W trakcie"
                      </Button>
                    </div>
                  </div>
